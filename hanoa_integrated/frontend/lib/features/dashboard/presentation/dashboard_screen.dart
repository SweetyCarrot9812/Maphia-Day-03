...
class DashboardScreen extends ConsumerWidget {
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        _buildActionButton(
                          ...
                          () => _createProblem(context),
                        ),
                      ],
                    ),
                    const SizedBox(height: 16),
                    Text(
                      '다양한 카테고리의 문제를 생성하고 관리할 수 있습니다.',
                      style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                        color: Colors.grey.shade600,
                      ),
                    ),
                    ...
                    Row(
                      children: [
                        _buildQuickActionCard(
                          ...
                          Colors.blue.shade600,
                          () => _createProblemWithCategory(context, 'clintest'),
                        ),
                        const SizedBox(width: 12),
                        _buildQuickActionCard(
                          context,
                          ...
                          Colors.green.shade600,
                          () => _createProblemWithCategory(context, 'lingumo'),
                        ),
                        const SizedBox(width: 12),
                        _buildQuickActionCard(
                          context,
                          ...
                          () => _createProblemWithCategory(context, 'areumfit'),
                        ),
                        const SizedBox(width: 12),
                        _buildQuickActionCard(
                          context,
                          ...
                          Colors.purple.shade600,
                          () => _createProblemWithCategory(context, 'haneultone'),
                        ),
                      ],
                    ),
                  ],
  ...
  void _createProblem(BuildContext context) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return const ProblemCreationDialog();
      },
    );
  }

  void _createProblemWithCategory(BuildContext context, String category) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return ProblemCreationDialog(initialCategory: category);
      },
    );
  }
...
// Problem Creation Dialog Widget
class ProblemCreationDialog extends StatefulWidget {
  final String? initialCategory;

  const ProblemCreationDialog({super.key, this.initialCategory});

  @override
  State<ProblemCreationDialog> createState() => _ProblemCreationDialogState();
}

class _ProblemCreationDialogState extends State<ProblemCreationDialog> {
  final _formKey = GlobalKey<FormState>();
  final _titleController = TextEditingController();
  final _descriptionController = TextEditingController();
  final _optionsController = List.generate(4, (index) => TextEditingController());
  ...
  void initState() {
    super.initState();
    _selectedCategory = widget.initialCategory ?? 'clintest';
  }

  final List<Map<String, String>> _categories = [
    {'id': 'clintest', 'name': 'Clintest (의료)'},
    {'id': 'lingumo', 'name': 'Lingumo (언어)'},
    {'id': 'areumfit', 'name': 'AreumFit (피트니스)'},
    {'id': 'haneultone', 'name': 'HaneulTone (성악)'},
  ];
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  const SizedBox(width: 12),
                  ElevatedButton(
                    onPressed: _isCreating ? null : _createProblem,
                    style: ElevatedButton.styleFrom(
  ...
  void _createProblem() async {
    ...
    try {
      // Simulate problem creation delay
      ...
      final problemData = {
        ...
        'options': _optionsController.map((controller) => controller.text.trim()).toList(),
        'correctAnswer': _correctAnswerIndex,
        'createdAt': DateTime.now().toIso8601String(),
      };

      // TODO: Send to backend or save locally
      print('Problem created: $problemData');
